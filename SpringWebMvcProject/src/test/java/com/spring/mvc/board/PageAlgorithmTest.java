...
public class PageAlgorithmTest {
	 ...
	 *** 페이징 알고리즘 만들기 ***
	 ...
	 - 총 게시물의 수가 142개이고, 현재 ㅅ용자가 12페이지에 머물러 있다면???
	 [이전] 11 12 13 14 15 
	 ...
	 한 화면에 부여줄 페이지 버튼이 10개씩이라면????
	 -> 끝 페이지 번호: 10번 
	 - 만약 현재 페이지가 36페이지고, 한 화면에 보여줄 페이지 버튼이
	 20개라면??
	 -> 끝 페이지 번호: 40번 

	 공식 : Math.ceil(현재 위치한 페이지 번호 / 한 화면에 보여질 페이질 버튼수) * 한 화면당 보여질 페이지 버튼수

	 # 4. 시작 페이지 번호 계산하기
	 - 현재 위치한 페이지가 15페이지고, 한 화면에 보여줄 페이지가 10페이지씩이라면???
	 -> 시작 페이지 번호: 11번
	 - 현재 위치한 페이지 73페이지이고, 한 화면에 보여줄 페이지가 20페이지라면?
	 -> 시작페이지 번호: 61번

	 공식: (끝 페이지 번호 - 한화면에 보여줄 페이지수)+1

	 #5. 끝 페이지 보정

	 - 총 게시물 수가 323개고 한페이지당 10개의 게시물을 보여줌.
	 - 그리고 이사람은 31페이지 보고있습니다.
	 - 그리고 한 화면에 버튼은 10개가 배치됩니다.
	 - 그렇다면, 위 공식에 의한 끝 페이지는 몇 번으로 계산되는가? -> 40번
	 - 하지만, 실제 끝 페이지는 몇번에서 끝나는가? -> 33번

	 # 5-1. 이전 버튼 활성 여부 설정.
	 - 언제 이전버튼을 활성/비활성화 시킬것인가의 여부 -> 시작페이지가 1인 부분에서 비활성, 나머지는 다 활성
	 공식: 시작페이지 번호가 1로 구해진 시점에서는 비활성 처리, 나머지는 활성

	  # 5-2. 다음 버튼 활성 여부 설정
	  - 언제 다음 버튼을 비활성화 할 것인가??
	  공식: 보정 전 끝 페이지 번호 * 한 페이지에 들어갈 게시물 수 >= 총게시물 수 --> 비활성
